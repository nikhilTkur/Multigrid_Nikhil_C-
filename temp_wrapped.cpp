...
public:
	//std::unordered_map<int, csr_matrix_elements> A_sp_list;
	//std::unordered_map<int, csr_jacobi_elements> A_jacobi_sp_list;
	//std::unordered_map<int, std::unordered_map<int, int>> topo_to_space_list;
	//std::unordered_map<int, std::vector<double>> b_list;
	//std::unordered_map<int, std::vector<std::tuple<int, int>>>parent_info_list;
	//std::unordered_map<int, std::unordered_map<int, std::vector<int>>>coarse_grid_edges_list;

	Eigen::SparseMatrix<double> coarsest_level_matrix;
	py::array_t<csr_matrix_elements> A_sp_list;
	py::array_t<csr_jacobi_elements> A_jacobi_sp_list;
	py::array_t<py::array_t<int>> topo_to_space_list;
	py::array_t<py::array_t<double>> b_list;
	py::array_t<py::array_t<py::array_t<int>>> parent_info_list;
	py::array_t<py::array_t<py::array_t<int>>> coarse_grid_edges_list;
	...
	ProblemVar(Eigen::SparseMatrix<double>& coarsest_level_matrix_py, py::array_t<csr_matrix_elements>& A_sp_list_py,
		...
		A_sp_list = A_sp_list_py;
		A_jacobi_sp_list = A_jacobi_sp_list_py;
		topo_to_space_list = topo_to_space_list_py;
		b_list = b_list_py;
		parent_info_list = parent_info_list_py;
		coarse_grid_edges_list = coarse_grid_edges_list_py;
	}	
};
...
Eigen::SparseMatrix<double> coarse_matrix_assemble(py::array_t<int>& cols, py::array_t<int>& rows,
	py::array_t<double>& vals, int& num_row) {
	std::vector<int>col_vec(cols.data(), cols.data() + cols.size());
	std::vector<int>row_vec(rows.data(), rows.data() + rows.size());
	std::vector<double>val_vec(vals.data(), vals.data() + vals.size());
	Eigen::SparseMatrix<double> sparse_mat(num_row, num_row);
	sparse_mat.reserve(Eigen::VectorXi::Constant(num_row, 4));
	for (int i = 0; i < row_vec.size()- 1; i++) {

		for (int j = row_vec[i]; j <= row_vec[i + 1] - 1; j++) {
			sparse_mat.insert(i, col_vec[j]) = vals[j];
		}
	}
	return sparse_mat;
}

std::vector<double> direct_solver(Eigen::SparseMatrix<double>& sparse_matrix, py::array_t<double>& b) {
...
void jacobirelaxation(cl::sycl::queue& q, std::vector<double>& vec, py::array_t<double>& b, ProblemVar& obj
	, int current_level) {
	int size = vec.size();
	...
	for (int i = 0; i < 10; i++) {
		R_V_mul_done = gemv(q, oneapi::mkl::transpose::nontrans, 1.0, obj.A_jacobi_sp_list[current_level].R_omega.matrix_handle,
			vec.data(), 0.0, prod_1.data());
		D_inv_F_done = gemv(q, oneapi::mkl::transpose::nontrans, omega, obj.A_jacobi_sp_list[current_level].D_inv.matrix_handle,
...
std::vector<double> interpolation2D(std::vector<double>vec_2h, ProblemVar& obj, int target_level) {
	...
	for (int i = 0; i < vec_h_dim; i++) {
		int fine_space_dof = obj.topo_to_space_list[target_level][i];
		if (std::get<0>(obj.parent_info_list[target_level][i]) == 0) {
			// Coarse topo dof and fine topo dof coincide
			vec_h[fine_space_dof] = vec_2h[obj.topo_to_space_list[target_level - 1][std::get<1>(obj.parent_info_list[target_level][i])]];
		}
		else {
			//Fine dof lies on coarse edge
			int edge_num = std::get<1>(obj.parent_info_list[target_level][i]);
			//Obtain corresponding edge vertices on topology
			int coarse_dof_1 = obj.topo_to_space_list[target_level - 1][obj.coarse_grid_edges_list[target_level - 1][edge_num][0]];
			int coarse_dof_2 = obj.topo_to_space_list[target_level - 1][obj.coarse_grid_edges_list[target_level - 1][edge_num][1]];
...
std::vector<double> restriction2D(std::vector<double>vec_h, ProblemVar& obj, int target_level) {
	...
	for (int i = 0; i < vec_2h_dim; i++) {
		vec_2h[obj.topo_to_space_list[target_level][i]] = vec_h[obj.topo_to_space_list[target_level + 1][i]];
	}
	return vec_2h;
}
...
std::vector<double> vcyclemultigrid(cl::sycl::queue& q, ProblemVar& obj,
	...
	if (current_level == coarsest_level) {		//USE DIRECT SOLVER HERE
		...
		gemv_A_vec_done = gemv(q, oneapi::mkl::transpose::nontrans, 1.0, obj.A_sp_list[current_level].matrix_handle,
...
std::vector<double> fullmultigrid(cl::sycl::queue& q, ProblemVar& object, std::vector<double>& f_h, int current_level) {
	...
	if (current_level == coarsest_level) {
		...
		vec_2h = fullmultigrid(q, object, object.b_list[current_level - 1], current_level - 1);
	}
...
std::vector<double> multigrid_solver(ProblemVar& object) {
	cl::sycl::queue q;
	std::vector<double> answer = fullmultigrid(q, object, object.b_list[finest_level], finest_level);
	return answer;
}

PYBIND11_MODULE(multigrid, m) {
}